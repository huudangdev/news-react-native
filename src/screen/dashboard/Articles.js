...
export default function Articles (props) {
  ...
  const data = newsReducer[category]
  ...
  async function onLoadMore () {
    if (!isLoadingMore) {
      ...
      const length = newsReducer[category].articles.length
      if (length < totalResults) {
        const currentPage = length / c.PAGESIZE // checks the current page
        const nextPage = currentPage + 1 // the next page

        try {
          const data = await api.getHeadlinesByCategory(category, nextPage)
