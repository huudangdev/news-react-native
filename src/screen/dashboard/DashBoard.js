...
export default function DashBoard (props) {
  ...
  const renderItem = (size = 'small', horizontal = false, grid = false, wrapper = true) => {
    return ({ item, index }) => {
      const article = new Article(item, navigate)
  ...
  const renderHorizontalItem = renderItem(null, true, false, true)

  const renderGridItem = renderItem('small', false, true, false)
  const renderHorizontalGridItem = renderItem(null, true, true, false)

  const renderSportItem = renderItem('large')
  const renderTechItem = renderItem('large', false, true)
  return (
